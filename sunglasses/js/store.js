...
const _t = {
  ar: {
    ...
    zonePlaceholder:'اختر منطقة التوصيل',
    shareLocation:'مشاركة موقعي الحالي',
...
function renderCustomSections() {
  ...
  if (!homePage) return;
  document.querySelectorAll('#homePage > .pb-custom-section').forEach(function(el) { el.remove(); });
  ...
  sections.forEach(function(sec, idx) {
    ...
    if (sec.type === 'hero') {
      ...
      var bgCss = sec.bgImage ? 'linear-gradient(rgba(0,0,0,.05),rgba(0,0,0,.05)),url(' + sec.bgImage + ') center/cover no-repeat' : (sec.bgColor || 'transparent');
      el.style.cssText = 'padding:30px 16px;background:' + bgCss + ';margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;min-height:1px';
      el.innerHTML = (sec.title ? '<h3 style="color:' + (sec.textColor || 'var(--text)') + ';font-size:1.2rem;font-weight:800;margin:0 0 18px;text-align:center">' + sec.title + '</h3>' : '') +
        '<div class="pb-products-placeholder" data-section="' + idx + '" style="text-align:center;padding:20px;color:var(--text-muted)"><i class="fa-solid fa-spinner fa-spin"></i> جاري تحميل المنتجات...</div>';
      setTimeout(function(sid) {
        var ph = document.querySelector('.pb-products-placeholder[data-section="' + sid + '"]');
        if (!ph) return;
        var secData = (JSON.parse(localStorage.getItem('mycart_marketing') || '{}').customSections || [])[sid];
        if (!secData) return;
        ...
        var cols = parseInt(secData.columns) || 4;
        var gap = secData.gap != null ? secData.gap : 16;
        var cardR = (secData.cardRadius != null ? secData.cardRadius : 12) + 'px';
        var shadowMap = { none:'0 0 0 transparent', sm:'0 1px 3px rgba(0,0,0,.08)', md:'0 4px 12px rgba(0,0,0,.1)', lg:'0 8px 25px rgba(0,0,0,.15)' };
        var cardShadow = shadowMap[secData.cardShadow] || shadowMap.sm;
        if (filtered.length === 0) { ph.innerHTML = '<div style="padding:30px;color:var(--text-muted);font-size:.85rem">لا توجد منتجات</div>'; return; }
        if (secData.layout === 'carousel') {
          var pHtml = '<div class="pb-carousel" style="display:flex;gap:' + gap + 'px;overflow-x:auto;scroll-snap-type:x mandatory;padding:4px 0;-webkit-overflow-scrolling:touch;scrollbar-width:none">';
          filtered.forEach(function(p) {
            var img = (p.images && p.images.length) ? p.images[0] : (p.image || '');
            var stars = secData.showRating && p.rating ? '<div style="font-size:.65rem;color:#f59e0b;margin-bottom:4px;direction:ltr;text-align:center">' + '★'.repeat(Math.round(p.rating)) + '</div>' : '';
            pHtml += '<div style="flex:0 0 calc(' + (100/cols) + '% - ' + gap + 'px);min-width:180px;scroll-snap-align:start;background:#fff;border-radius:' + cardR + ';overflow:hidden;border:1px solid var(--border);box-shadow:' + cardShadow + ';transition:transform .2s" onclick="openProductDetail(' + p.id + ')">' +
              '<img src="' + img + '" alt="' + (p.name||'') + '" style="width:100%;height:180px;object-fit:cover;display:block">' +
              '<div style="padding:12px;text-align:center">' +
              stars +
              (secData.showTitle !== false ? '<div style="font-size:.8rem;font-weight:700;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + (p.name||'') + '</div>' : '') +
              (secData.showPrice !== false ? '<div style="font-size:.9rem;font-weight:800;color:var(--accent)">' + (typeof p.price === 'number' ? p.price.toFixed(2) : p.price) + ' ' + (window._currency || '₽') + '</div>' : '') +
              (secData.showAddToCart !== false ? '<button onclick="event.stopPropagation();addToCart(' + p.id + ')" style="width:100%;margin-top:8px;padding:8px;border:none;border-radius:8px;background:var(--accent);color:#fff;font-weight:700;cursor:pointer;font-size:.7rem;font-family:inherit">🛒 أضف للسلة</button>' : '') +
              '</div></div>';
          });
          pHtml += '</div>';
          ph.innerHTML = pHtml;
        } else {
          var pHtml = '<div style="display:grid;grid-template-columns:repeat(' + cols + ',1fr);gap:' + gap + 'px;padding:0">';
          filtered.forEach(function(p) {
            var img = (p.images && p.images.length) ? p.images[0] : (p.image || '');
            var stars = secData.showRating && p.rating ? '<div style="font-size:.65rem;color:#f59e0b;margin-bottom:4px;direction:ltr;text-align:center">' + '★'.repeat(Math.round(p.rating)) + '</div>' : '';
            pHtml += '<div style="background:#fff;border-radius:' + cardR + ';overflow:hidden;border:1px solid var(--border);box-shadow:' + cardShadow + ';transition:transform .2s" onclick="openProductDetail(' + p.id + ')">' +
              '<img src="' + img + '" alt="' + (p.name||'') + '" style="width:100%;height:180px;object-fit:cover;display:block">' +
              '<div style="padding:12px;text-align:center">' +
              stars +
              (secData.showTitle !== false ? '<div style="font-size:.8rem;font-weight:700;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + (p.name||'') + '</div>' : '') +
    ...
    } else if (sec.type === 'marquee') {
      var speedMap = { slow: 20, medium: 12, fast: 6 };
      var dur = speedMap[sec.speed] || 12;
      var dir = sec.direction === 'ltr' ? 'normal' : 'reverse';
      var sizeMap = { sm: '.85rem', md: '1.15rem', lg: '1.5rem' };
      var fs = sizeMap[sec.textSize] || '1.15rem';
      el.style.cssText = 'padding:14px 0;background:' + (sec.bg || '#1e293b') + ';margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;overflow:hidden';
      var text = sec.text || '';
      var repCount = sec.repeat !== false ? 6 : 1;
      var repeatHtml = Array(repCount).fill(text).join(' &nbsp;&nbsp;&nbsp; ');
      var mid = 'pbMarquee_' + idx;
      el.innerHTML = '<div id="' + mid + '" style="white-space:nowrap;overflow:hidden;font-size:' + fs + ';color:' + (sec.textColor || '#fff') + ';font-weight:700">' +
        '<div style="display:inline-block;padding-left:100%;animation:pbMarquee ' + dur + 's linear infinite;animation-direction:' + dir + '">' + repeatHtml + '</div></div>';
      if (!document.getElementById('pbMarqueeStyle')) {
        var s = document.createElement('style'); s.id = 'pbMarqueeStyle'; s.textContent = '@keyframes pbMarquee{0%{transform:translateX(0)}100%{transform:translateX(-100%)}}'; document.head.appendChild(s);
      }
    } else if (sec.type === 'logos') {
      var images = sec.images || [];
      var speedMap = { slow: 25, medium: 15, fast: 8 };
      var dur = speedMap[sec.speed] || 15;
      var h = sec.height || 60;
      var logBg = sec.bg || '#ffffff';
      var grayscale = sec.grayscale !== false;
      var linkMode = sec.linkMode || 'none';
      el.style.cssText = 'padding:20px 0;background:' + logBg + ';margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;overflow:hidden';
      if (!images.length) images = [{name:'شعار 1'},{name:'شعار 2'},{name:'شعار 3'},{name:'شعار 4'},{name:'شعار 5'}];
      var filterStyle = grayscale ? 'filter:grayscale(1);opacity:.7' : '';
      function logoLink(logoHtml, img) {
        if (linkMode === 'custom' && img.link) return '<a href="' + img.link + '" target="_blank" style="text-decoration:none;display:contents">' + logoHtml + '</a>';
        if (linkMode === 'brand') {
          return '<a href="javascript:filterCategory(\'' + (img.name||'').replace(/'/g,"\\'") + '\')" style="text-decoration:none;display:contents">' + logoHtml + '</a>';
        }
        return logoHtml;
      }
      var allItems = images.concat(images).map(function(img) {
        var inner = img.src ? '<div style="flex-shrink:0;height:' + h + 'px;display:flex;align-items:center;padding:0 24px;' + filterStyle + '"><img src="' + img.src + '" alt="' + (img.name||'') + '" style="max-height:' + h + 'px;max-width:120px;object-fit:contain"></div>'
          : '<div style="flex-shrink:0;height:' + h + 'px;width:100px;display:flex;align-items:center;justify-content:center;' + filterStyle + ';background:linear-gradient(135deg,#f1f5f9,#e2e8f0);border-radius:8px;color:#94a3b8;font-size:.7rem;font-weight:700">' + (img.name||'شعار') + '</div>';
        return logoLink(inner, img);
      }).join('');
      el.innerHTML = '<div style="overflow:hidden"><div style="display:flex;animation:pbLogosScroll ' + dur + 's linear infinite">' + allItems + '</div></div>';
      if (!document.getElementById('pbLogoScrollStyle')) {
        var s2 = document.createElement('style'); s2.id = 'pbLogoScrollStyle'; s2.textContent = '@keyframes pbLogosScroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}'; document.head.appendChild(s2);
      }
    } else if (sec.type === 'counters') {
      var items = sec.items || [];
      var cols = parseInt(sec.columns) || 4;
      var ic = sec.iconColor || '#ef4444';
      var tc = sec.textColor || '#fff';
      var bg = sec.bg || '#0f172a';
      var anim = sec.animation !== false;
      el.style.cssText = 'padding:50px 16px;background:' + bg + ';margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;';
      var cHtml = '<div style="display:grid;grid-template-columns:repeat(' + cols + ',1fr);gap:24px">';
      items.forEach(function(c, ci) {
        var val = c.value || 0;
        var suffix = c.suffix || '';
        var icon = c.icon || '⭐';
        var isFa = icon.indexOf('fa-') === 0;
        var cid = 'pbCounter_' + idx + '_' + ci;
        cHtml += '<div style="text-align:center;padding:10px">' +
          '<div style="font-size:2.2rem;margin-bottom:6px;color:' + ic + '">' + (isFa ? '<i class="fa-solid ' + icon + '"></i>' : icon) + '</div>' +
          '<div id="' + cid + '" style="font-size:2.2rem;font-weight:900;color:' + ic + ';line-height:1.2">' + (anim ? '0' : val + suffix) + '</div>' +
          '<div style="font-size:.85rem;color:' + tc + ';opacity:.7;margin-top:4px">' + (c.label||'') + '</div></div>';
      });
      cHtml += '</div>';
      el.innerHTML = cHtml;
      if (anim && items.length) {
        (function(targetIdx, cbItems, ic2, dur) {
          var started = false;
          function animCounters() {
            if (started) return;
            var rect = (document.getElementById('pbCounter_' + targetIdx + '_0') || {}).getBoundingClientRect();
            if (!rect || rect.top > window.innerHeight || rect.bottom < 0) return;
            started = true;
            cbItems.forEach(function(c, ci) {
              var el2 = document.getElementById('pbCounter_' + targetIdx + '_' + ci);
              if (!el2) return;
              var target = parseFloat(c.value) || 0;
              var suffix = c.suffix || '';
              var duration = 2000;
              var steps = 30;
              ...
              var interval = setInterval(function() {
                ...
                el2.textContent = (Number.isInteger(target) ? Math.round(cur) : cur.toFixed(1)) + suffix;
              }, duration / steps);
            });
          }
          window.addEventListener('scroll', animCounters);
          setTimeout(animCounters, 500);
        })(idx, items, ic, 2000);
      }
    } else if (sec.type === 'trust') {
      var items = sec.items || [];
      var lay = sec.layout || 'row';
      var ic = sec.iconColor || '#ef4444';
      var tc = sec.textColor || '#1e293b';
      var bg = sec.bg || '#ffffff';
      el.style.cssText = 'padding:30px 16px;background:' + bg + ';margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;';
      if (lay === 'cards') {
        var tHtml = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:20px">';
        items.forEach(function(it) {
          var icon = it.icon || '🛡️';
          var isFa = icon.indexOf('fa-') === 0;
          tHtml += '<div style="border:1px solid var(--border);border-radius:14px;padding:24px;text-align:center;background:var(--card);box-shadow:0 2px 12px rgba(0,0,0,.04);transition:transform .25s,box-shadow .25s">' +
            '<div style="font-size:2.2rem;margin-bottom:10px;color:' + ic + '">' + (isFa ? '<i class="fa-solid ' + icon + '"></i>' : icon) + '</div>' +
            '<h4 style="font-size:1rem;font-weight:800;margin:0 0 6px;color:' + tc + '">' + (it.title||'') + '</h4>' +
            (it.desc ? '<p style="font-size:.78rem;line-height:1.6;color:var(--text-muted);margin:0">' + it.desc + '</p>' : '') + '</div>';
        });
        tHtml += '</div>';
        el.innerHTML = tHtml;
      } else {
        var tHtml = '<div style="display:grid;grid-template-columns:repeat(' + Math.min(items.length, 6) + ',1fr);gap:16px">';
        items.forEach(function(it) {
          var icon = it.icon || '🛡️';
          var isFa = icon.indexOf('fa-') === 0;
          tHtml += '<div style="text-align:center;padding:16px 8px">' +
            '<div style="font-size:2rem;margin-bottom:6px;color:' + ic + '">' + (isFa ? '<i class="fa-solid ' + icon + '"></i>' : icon) + '</div>' +
            '<div style="font-size:.9rem;font-weight:800;color:' + tc + '">' + (it.title||'') + '</div>' +
            (lay === 'grid' && it.desc ? '<div style="font-size:.72rem;color:var(--text-muted);margin-top:4px">' + it.desc + '</div>' : '') + '</div>';
        });
        tHtml += '</div>';
        el.innerHTML = tHtml;
      }
      ...
      el.style.cssText = 'padding:30px 16px;background:' + (sec.bg || '#ffffff') + ';margin:0 auto;max-width:900px;width:100%;box-sizing:border-box;';
      var tbl = '<div style="overflow-x:auto;border-radius:12px;border:' + (border ? '1px solid #e2e8f0' : 'none') + ';box-shadow:0 2px 12px rgba(0,0,0,.05)">' +
        '<table style="width:100%;border-collapse:collapse;text-align:center;font-size:.85rem">';
      if (headers.length) {
        tbl += '<thead><tr>' + headers.map(function(h) {
          return '<th style="padding:12px 14px;font-weight:800;color:#fff;background:' + hBg + ';' + bStyle + '">' + h + '</th>';
        }).join('') + '</tr></thead>';
      }
      tbl += '<tbody>' + rows.map(function(r, ri) {
        var cells = (r.cells||'').split(',').map(function(s){return s.trim()});
        var rowStyle = (ri % 2 && striped ? 'background:#f8fafc;' : '');
        return '<tr style="' + rowStyle + '">' + cells.slice(0, Math.max(headers.length,1)).map(function(c) {
          return '<td style="padding:10px 14px;color:' + tc + ';' + bStyle + '">' + c + '</td>';
        }).join('') + '</tr>';
      }).join('') + '</tbody></table></div>';
      el.innerHTML = (sec.title ? '<h3 style="font-size:1.2rem;font-weight:800;margin:0 0 16px;text-align:center;color:' + tc + '">' + sec.title + '</h3>' : '') + tbl;
...
function reorderHomeSections() {
  ...
  // Render custom sections first (they get inserted as DOM elements)
...
function applyMarketing() {
  ...
  if (annBar) {
    ...
    if (showAnnounce && announceText) {
      ...
      if (isRotate && texts.length > 1) {
        ...
        annBar.innerHTML = `<div class="announcement-ticker" style="text-align:center;width:100%;${pulseStyle}">${displayText}</div>`;
      } else {
        /* ── seamless marquee (also used for slide) ── */
      ...
      if (isStatic || isRotate) annBar.style.justifyContent = 'center'; else annBar.style.justifyContent = '';
    } else {
  ...
  if (se.enabled && se.type) {
    ...
    const dir = se.direction || 'top';
    const qty = Math.min(100, Math.max(10, parseInt(se.quantity) || 35));
    const sizeMul = { small: 0.6, medium: 1, large: 1.5 }[se.size || 'medium'];
    const speedMul = { slow: 0.5, medium: 1, fast: 1.6 }[se.speed || 'medium'];
    const count = se.type === 'ramadan' ? 0 : qty;
    if (se.type === 'ramadan') {
    ...
    for (let i = 0; i < count; i++) {
      ...
      p.className = isHoriz ? (dir === 'left' ? 'se-h-left' : 'se-h-right') : (clsMap[se.type] || 'fall-snow');
      ...
      const rev = (!isHoriz && dir === 'bottom') ? 'animation-direction:reverse;' : '';
      const snowStyle = se.type==='snow' ? `width:${size*sizeMul}px;height:${size*sizeMul}px;background:rgba(255,255,255,.8);border-radius:50%;box-shadow:0 0 ${size*sizeMul}px rgba(255,255,255,.4)` : 'color:rgba(255,255,255,.8)';
      p.style.cssText = `left:${pos}%;font-size:${size*sizeMul}px;animation-delay:${delay}s;animation-duration:${dur/speedMul}s;${snowStyle};${rev}`;
      ...
      slider.innerHTML = banners.map(b => {
        ...
        if (b.showMiniCard && b.link && b.link.includes('openDetail(')) {
          ...
          if (prodIdMatch) {
            ...
            if (prod) {
              const finalPrice = isWholesale ? Math.round(prod.price * 0.85) : prod.price;
...
function filterCategory(cat, updateHash = true) {
  ...
  document.querySelectorAll('#homePage > .pb-custom-section').forEach(function(el) {
...
function _productCardHtml(p) {
  ...
  return `<div class="product-card${_isOut ? ' out-of-stock' : ''}" data-id="${p.id}" onclick="openDetail(${p.id})">
      ...
      <div class="product-info">
        <h3>${p.name}</h3>
        <div class="price">${(()=>{const _op=calcOfferPrice(p);if(_op!==null)return `${CURRENCY}${_op} <span class="old-price">${CURRENCY}${isWholesale?Math.round(p.price*0.85):p.price}</span>`;const _bp=isWholesale?Math.round(p.price*0.85):p.price;return `${CURRENCY}${_bp}${p.oldPrice?` <span class="old-price">${CURRENCY}${p.oldPrice}</span>`:''}`;})()}</div>
        ${p.brand ? `<div class="product-brand"><i class="fa-solid fa-award"></i> ${p.brand}</div>` : ''}
      </div>
      <div class="quick-add" onclick="event.stopPropagation();${_isOut ? '' : 'quickAdd(' + p.id + ',this)'}" style="${_isOut ? 'opacity:.3;pointer-events:none' : ''}"><i class="fa-solid fa-cart-shopping"></i></div>
    </div>`;
}
    ...
    if (offScroll) {
      offScroll.innerHTML = offerProducts.map(p => {
        ...
        return `<div class="mini-card" data-id="${p.id}" onclick="openDetail(${p.id})">
          ...
          <img src="${getProductImages(p)[0]}" alt="${p.name}" loading="lazy">
          <div class="info"><h4>${p.name}</h4><div class="p">${(()=>{const _op=calcOfferPrice(p);if(_op!==null)return `${CURRENCY}${_op} <span style="text-decoration:line-through;opacity:.7;font-size:.7rem">${CURRENCY}${isWholesale?Math.round(p.price*0.85):p.price}</span>`;const _bp=isWholesale?Math.round(p.price*0.85):p.price;return `${CURRENCY}${_bp}`;})()}</div></div>
  ...
  if (featured.length && showFeaturedSection && isSectionEnabled('featured')) {
    ...
    if (featScroll) {
      featScroll.innerHTML = featured.map(p => {
        ...
        const finalPrice = isWholesale ? Math.round(p.price * 0.85) : p.price;
        ...
        return '<div class="mini-card" data-id="' + p.id + '" onclick="openDetail(' + p.id + ')">' +
          ...
          '<div class="feat-body">' +
            ...
            '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">' +
              '<span class="feat-price">' + CURRENCY + finalPrice + '</span>' +
              oldPriceHtml +
            '</div>' +
            '<button class="feat-add" onclick="event.stopPropagation();quickAdd(' + p.id + ', this)"><i class="fa-solid fa-cart-plus"></i> ' + __('quickAdd') + '</button>' +
          '</div>' +
        '</div>';
      }).join('');
    }
...
function loadBannerSettings() {
  const mkt = JSON.parse(localStorage.getItem('mycart_marketing') || '{}');
  if (mkt.bannerSettings) return mkt.bannerSettings;
...
function startBannerAutoScroll() {
  ...
  if (settings.layout === 'grid') {
    ...
    slider.querySelectorAll('.banner-slide').forEach((slide, index) => {
      if (index === 0) {
        ...
        slide.style.flex = '1 1 calc(50% - 8px)';
    ...
    // default/slider
    const isFade = settings.sliderEffect === 'fade' || settings.sliderEffect === 'zoom';
    if (isFade) {
      ...
      slider.querySelectorAll('.banner-slide').forEach((slide, index) => {
        ...
        slide.style.transition = settings.sliderEffect === 'zoom' 
  ...
  bannerScrollInterval = setInterval(() => {
    ...
    const isFade = settings.layout === 'slider' && (settings.sliderEffect === 'fade' || settings.sliderEffect === 'zoom');
    if (isFade) {
...
function openDetail(id, fromRoute) {
  ...
  if (offerPrice !== null) {
    const baseP = isWholesale ? Math.round(p.price * 0.85) : p.price;
    document.getElementById('detailPrice').innerHTML = `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      <span style="font-size:1.5rem;font-weight:900;color:var(--accent)">${CURRENCY}${offerPrice}</span>
    ...
    document.getElementById('detailPrice').innerHTML = `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      <span style="font-size:1.5rem;font-weight:900;color:var(--accent)">${CURRENCY}${isWholesale ? Math.round(p.price * 0.85) : p.price}</span>
      <span style="font-size:.95rem;text-decoration:line-through;color:#94a3b8;font-weight:500">${CURRENCY}${p.oldPrice}</span>
      ${getProductDiscount(p) ? `<span style="font-size:.7rem;font-weight:800;background:#ef4444;color:#fff;padding:3px 10px;border-radius:999px;line-height:1">-${getProductDiscount(p)}%</span>` : ''}
    </div>`;
  } else {
    document.getElementById('detailPrice').innerHTML = `<span style="font-size:1.5rem;font-weight:900;color:var(--accent)">${CURRENCY}${isWholesale ? Math.round(p.price * 0.85) : p.price}</span>`;
  }
...
const discVal = marketingData.fbt?.discount || 10;
      ...
      const prodHtml = allItems.map((item, idx) => {
        ...
        return '<div style="text-align:center;width:70px;cursor:pointer" onclick="openDetail(' + item.id + ')">' +
          '<div style="position:relative;display:inline-block">' +
            '<img src="' + imgSrc + '" style="width:46px;height:46px;border-radius:8px;object-fit:cover;border:1px solid var(--border)' + (isOut ? ';opacity:.4' : '') + '">' +
  ...
  function updateOptionPrice(prod) {
    ...
    const finalBase = offerPrice !== null ? offerPrice : base;
    const total = finalBase + totalExtra;
    const opOffer = getProductOffer(prod);
    let html = `<span style="font-size:1.2rem">${CURRENCY}${total}</span>`;
    if (offerPrice !== null) {
  ...
  if (related.length) {
    ...
    document.getElementById('relatedScroll').innerHTML = related.map(r => `
      <div class="mini-card" data-id="${r.id}" onclick="openDetail(${r.id})">
        <img src="${getProductImages(r)[0]}" alt="${r.name}" loading="lazy">
...
function renderOrders() {
  ...
  const ORDER_STATUS_MAP = {
    pending: { label: __('statusPending'), bg: '#fef3c7', text: '#92400e', icon: 'fa-clock' },
  ...
  document.getElementById('orderPageBody').innerHTML = `
    ...
    <div style="background:#f8fafc;border-radius:10px;padding:14px;margin-bottom:16px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:.85rem">
        <div><span style="color:var(--text-muted)">${__('name')}</span><br><strong id="opName">${o.customer?.name || '—'}</strong> ${o.customer?.name ? `<button onclick="copyText(document.getElementById('opName').textContent,'${__('name')}')" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:.75rem;margin-right:4px"><i class="fa-regular fa-copy"></i></button>` : ''}</div>
        <div><span style="color:var(--text-muted)">${__('phone')}</span><br><strong dir="ltr" style="display:inline-block" id="opPhone">${o.customer?.phone || '—'}</strong> ${o.customer?.phone ? `<button onclick="copyText(document.getElementById('opPhone').textContent,'${__('phone')}')" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:.75rem"><i class="fa-regular fa-copy"></i></button>` : ''}</div>
        <div><span style="color:var(--text-muted)">${__('city')}</span><br><strong id="opCity">${o.customer?.city || '—'}</strong> ${o.customer?.city ? `<button onclick="copyText(document.getElementById('opCity').textContent,'${__('city')}')" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:.75rem"><i class="fa-regular fa-copy"></i></button>` : ''}</div>
        <div><span style="color:var(--text-muted)">${__('address')}</span><br><strong id="opAddr">${o.customer?.address || '—'}</strong> ${o.customer?.address ? `<button onclick="copyText(document.getElementById('opAddr').textContent,'${__('address')}')" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:.75rem"><i class="fa-regular fa-copy"></i></button>` : ''}</div>
        ${o.deliveryZone ? `<div><span style="color:var(--text-muted)">${__('deliveryZone')}</span><br><strong>${o.deliveryZone}</strong></div>` : ''}
      </div>
...
function adminLoadForm(p) {
  ...
  document.getElementById('admin-addProduct').innerHTML = `
    ...
    <form onsubmit="adminSaveProduct(event)">
      ...
      <div style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;padding:14px;background:var(--card-bg)">
        <div style="font-size:.8rem;font-weight:700;margin-bottom:10px;color:var(--text-muted)"><i class="fa-solid fa-layer-group"></i> الخيارات (لون، مقاس، ...)</div>
        <div id="apOptions">
...
function getDefaultAppearance() {
  return {
    ...
    stickyHeader:false,showSearch:true,showWishlist:true,headerFrom:'#ef4444',headerTo:'#dc2626',headerText:'#ffffff',headerPadding:40,
...
function adminSaveMarketing(subTab = 'seo') {
  ...
  if (subTab === 'seo') {
    ...
    currentData.seasonalEffect = {
      ...
      direction: document.getElementById('admMktSeasonalDir').value,
      quantity: parseInt(document.getElementById('admMktSeasonalQty').value) || 35,
      size: document.getElementById('admMktSeasonalSize').value,
      speed: document.getElementById('admMktSeasonalSpeed').value
    };
...
function calcOfferPrice(p) {
  const o = getProductOffer(p);
  if (!o) return null;
  const base = isWholesale ? Math.round(p.price * 0.85) : p.price;
    ...
    return '<div class="flash-card' + (_isOut ? ' out-of-stock' : '') + '" data-id="' + p.id + '" onclick="openDetail(' + p.id + ')">' +
      ...
      '<div class="flash-card-body">' +
        ...
        '<div class="flash-card-prices">' +
          '<span class="flash-current">' + CURRENCY + (p.price || 0).toFixed(2) + '</span>' +
        ...
        '<button class="flash-add-btn" onclick="' + (_isOut ? '' : 'event.stopPropagation();quickAdd(' + p.id + ',this);') + '" style="' + (_isOut ? 'opacity:.3;pointer-events:none' : '') + '"><i class="fa-solid fa-cart-shopping"></i> ' + (_isOut ? 'نفذ' : __('quickAdd')) + '</button>' +
      '</div>' +
    '</div>';
  }).join('');
...
initFlashSales();
// Dynamically shrink sticky header on scroll down
