        ...
        if (Array.isArray(imported)) {
          ...
          imported.forEach(function(p) {
            if (p.name && p.price !== undefined) {
              ...
              var addCategoryIfMissing = function(catName, isBrand) {
                if (catName && typeof catName === 'string') {
                  var trimmed = catName.trim();
                  if (trimmed && !cats.some(function(c) { return c.name === trimmed; })) {
                    cats.push({ name: trimmed, image: '', isBrand: isBrand });
                    catsModified = true;
                  }
                }
              };
              ...
              if (Array.isArray(p.categories)) {
                p.categories.forEach(function(c) { addCategoryIfMissing(c, false); });
              }
              if (p.brand) {
...
+ '<div class="search-bar">'
  ...
  + '<input type="text" id="adminStandaloneProdSearch" oninput="renderProducts()" value="'+searchQ+'" placeholder="بحث باسم المنتج..." style="width:100%;padding:10px 40px 10px 14px;border:1.5px solid var(--border);border-radius:10px;font-family:inherit;font-size:.85rem;box-sizing:border-box">'
  + '</div>';
...
function toggleMktSubMenu(e) { var btn = e.currentTarget; var sub = btn.nextElementSibling; if (sub && sub.classList) sub.classList.toggle('show'); }
...
function handleHashChangeAdmin() {
  var hash = (location.hash || '#dashboard').replace('#', '').split('/')[0];
